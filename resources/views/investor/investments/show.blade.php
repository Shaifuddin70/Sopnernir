<x-app-layout>
    ...
    <div class="mx-auto space-y-6 p-2 sm:p-8">
        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-2">{{ __('My profit by month (this pool)') }}</h3>
            <p class="text-sm text-gray-600 mb-4">{{ __('Your share each month on this pool.') }}</p>
        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            ...
            <ul class="divide-y border rounded-md text-sm">
                ...
                @empty
                    <li class="px-4 py-3 text-gray-500">{{ __('No documents uploaded.') }}</li>
                @endforelse
            </ul>
        </div>

        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-4">{{ __('Co-investors') }}</h3>
            @if ($investment->participants->isEmpty())
                <p class="text-sm text-gray-500">{{ __('No participants on this pool yet.') }}</p>
            @else
                        ...
                        <tbody>
                            @foreach ($investment->participants as $p)
                                <tr class="border-b border-gray-100">
                                    <td class="py-2 pr-4 font-medium text-gray-900">{{ $p->user->name }}</td>
